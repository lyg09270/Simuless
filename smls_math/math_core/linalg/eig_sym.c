...
void smls_mat_eig_sym(const float* const AA, float* ev, float* d, uint16_t row)
{
    float e[row];
...
static void tridiag(float* a, uint16_t row, float* d, float* e)
{
    ...
    *(d + 0) = 0.0f;
    *(e + 0) = 0.0f;
    /* Contents of this loop can be omitted if eigenvectors not wanted except for statement
     * d[i]=a[i][i]; */
    for (i = 0; i < row; i++)
    {
        l = i;
                ...
                for (i = m - 1; i >= l; i--)
                {
                    ...
                    *(d + i + 1) = g + (p = s * r);
                    ...
                    for (k = 0; k < row; k++)
                    {
                        ...
                        *(z + row * k + i + 1) =
                            s * *(z + row * k + i) + c * f; // z[k][i + 1] = s * z[k][i] + c * f;
                        *(z + row * k + i) =
                            c * *(z + row * k + i) - s * f; // z[k][i] = c * z[k][i] - s * f;
                    }
                }
                if (r == 0.0f && i >= l)
                    continue;
