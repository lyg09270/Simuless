                      ...
                      bool random_guess_active)
{
    ...
    if (random_guess_active)
    {
        ...
        for (uint32_t i = 0; i < random_iterations; i++)
        {
            ...
            {
                x[j] = difference_value * ((float)rand() / (float)RAND_MAX) + min_value;
            }
    ...
    for (uint32_t i = 0; i < random_iterations; i++)
    {
        ...
        past_gradients[gradient_index] = (uint8_t)(alpha * smls_mat_norm(dx, 1, elements, 2));
...
static float check_solution(float dx[], float x[], float* best_sqrt_sum_dx, float best_x[],
                            uint8_t* elements)
{
    // Do L2-norm on dx
    float sqrt_sum_dx = smls_mat_norm(dx, 1, *elements, 2);
