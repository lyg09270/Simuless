                ...
                bool only_compute_R)
{
    ...
    float s, Rk, r, W[row_a], WW[row_a_row_a], Hi[row_a_row_a], H[row_a_row_a], HiH[row_a_row_a],
        HiR[row_a * column_a];
    ...
    for (uint16_t k = 0; k < l; k++)
    {
        ...
        // WW = W*W'
        smls_mat_mul(WW, W, W, row_a, 1, 1, row_a);
        ...
        if (!only_compute_R)
        {
            smls_mat_mul(HiH, Hi, H, row_a, row_a, row_a, row_a);
            memcpy(H, HiH, row_a_row_a * sizeof(float));
        }

        // HiR = Hi * R -> HiR = R
        smls_mat_mul(HiR, Hi, R, row_a, row_a, row_a, column_a);
