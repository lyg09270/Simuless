 ...
 *
 * @note All matrices must have the same dimensions: row × column.
 */
void smls_mat_add(float* C, const float* const A, const float* const B, uint16_t row,
                  uint16_t column);
 ...
 * @param[out] Ai output matrix Ai
 * @param[in]  A Input matrix A[n*n]
 * @param[in]  row Number of rows and columns in A
 * @retval 0 Success
 * @retval -ENOTSUP Inverse not possible with given matrix
 **/
int smls_mat_inv(float* Ai, const float* const A, uint16_t row);

// TODO:add test for smls_linsolve_upper_triangular
/**
 ...
 *   m == n
 * @param[in] A Input matrix A
 * @param[out] x Output vector X
 * @param[in] b Input matrix/vector b
 * @param[in] column Number of rows and columns in A
 */
void smls_linsolve_upper_triangular(const float* const A, float* x, const float* const b,
                                    uint16_t column);
 ...
 * @details
 *   At = A'
 * @param[out] At Output matrix [n*m]
 * @param[in]  A Input matrix [m*n]
 * @param[in]  row Number of rows in A and columns in At
 * @param[in]  column Number of columns in A and rows in At
 **/
void smls_mat_transpose(float* At, const float* const A, uint16_t row, uint16_t column);

/**
 * @brief C = A * B
 * @param[out] C Output Matrix [row_a*column_b]
 * @param[in]  A Input matrix [row_a*column_a]
 * @param[in]  B Input matrix [column_a*column_b]
 * @param[in]  row_a Number of rows in A
 * @param[in]  column_a Number of columns in A
 * @param[in]  row_b Number of rows in B
 * @param[in]  column_b Number of columns in B (and rows in C)
 **/
int smls_mat_mul(float* C, const float* const A, const float* const B, uint16_t row_a,
                 uint16_t column_a, uint16_t row_b, uint16_t column_b);
 ...
 **/
int smls_mat_qr(const float* const A, float* Q, float* R, uint16_t row_a, uint16_t column_a,
                bool only_compute_R);
 ...
 * @param row Number of rows and columns
 **/
void smls_linsolve_lower_triangular(const float* const A, float* x, const float* const b,
                                    uint16_t row);
 ...
 **/
int smls_mat_lup(const float* const A, float* LU, uint8_t* P, uint16_t row);
 ...
 **/
float smls_mat_det(const float* const A, uint16_t row);
 ...
 **/
int smls_linsolve_lup(const float* const A, float* x, const float* const b, uint16_t row);
 ...
 * @param row Number of rows and columns in A
 **/
void smls_mat_chol(const float* const A, float* L, uint16_t row);
 ...
 **/
void smls_mat_chol_update(float* L, const float* const x, uint16_t row, bool rank_one_update);
 ...
 * @param column Number of columns in A
 **/
void smls_mat_pinv(float* Ai, const float* const A, uint16_t row, uint16_t column);
 ...
 **/
int smls_mat_hankel(const float* const V, float* H, uint16_t row_v, uint16_t column_v,
                    uint16_t row_h, uint16_t column_h, uint16_t shift);
 ...
 * @param row Number of rows and columns in A
 **/
void smls_mat_balance(float A[], uint16_t row);
/**
 ...
 * @param row Number of rows in A
 **/
void smls_mat_eig(const float* const A, float* wr, float* wi, uint16_t row);
/**
 ...
 * @param row Number of rows in A
 **/
void smls_mat_eig_sym(const float* const A, float* ev, float* d, uint16_t row);
/**
 ...
 * @param l Type of sum
 **/
void smls_mat_sum(float* Ar, const float* const A, uint16_t row, uint16_t column, uint8_t l);
/**
 ...
 **/
float smls_mat_norm(const float* const A, uint16_t row, uint16_t column, uint8_t l);
/**
 ...
 **/
void smls_mat_expm(const float* const A, float* exp, uint16_t row);

/**
 * @brief Solve a nonlinear equation system
 * @details
 *   Solve:
 *
 *   f(x) = b
 ...
 *   dx = f(x) - b
 *
 *   The solution is updated iteratively as:
 *
 *   x = x - alpha * dx
 *
 *   If random_guess_active is enabled,
 *   the solver first performs multiple random
 *   initial guesses in the range:
 *
 *   [min_value, max_value]
 *
 *   and selects the best initial solution
 ...
 *   dx [elements]
 *
 * @param nonlinear_equation_system
 *   Callback function that computes the residual vector dx
 *
 * @param b Target vector
 *
 * @param x Input initial guess and output solution vector
 *
 * @param elements Number of variables
 *
 * @param alpha Iteration step size
 *
 * @param max_value Maximum random initial guess
 *
 * @param min_value Minimum random initial guess
 *
 * @param random_guess_active
 *   Enable random initial search
 **/
void smls_nonlinsolve(void (*nonlinear_equation_system)(float[], float[], float[]), float b[],
                      float x[], uint8_t elements, float alpha, float max_value, float min_value,
 ...
 **/
void smls_linsolve_gauss(const float* const A, float* x, const float* const b, uint16_t row,
